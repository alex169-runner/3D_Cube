...
public class PLLTableEditor : EditorWindow
{
    private PLLTableCreator tableCreator;
    private PLLStateTable currentTable;
    private bool isBuilding = false;

    [MenuItem("CubeSolver/Build PLL Table")]
    static void ShowWindow()
    {
        GetWindow<PLLTableEditor>("PLL Table Builder");
    }

    void OnGUI()
    {
        GUILayout.Label("PLL状态表构建工具", EditorStyles.boldLabel);

        EditorGUILayout.Space();

        if (GUILayout.Button("构建新的PLL状态表", GUILayout.Height(30))) {
            BuildPLLTable();
        }
    ...
    void BuildPLLTable()
    {
        if (tableCreator == null) {
            ...
            tableCreator = tempGO.AddComponent<PLLTableCreator>();
        }
