...
public class CrossTableEditor : EditorWindow
{
    private CrossTableCreator tableCreator;
    private CrossStateTable currentTable;
    private bool isBuilding = false;

    [MenuItem("CubeSolver/Build Cross Table")]
    static void ShowWindow()
    {
        GetWindow<CrossTableEditor>("Cross Table Builder");
    }

    void OnGUI()
    {
        GUILayout.Label("Cross状态表构建工具", EditorStyles.boldLabel);

        EditorGUILayout.Space();

        if (GUILayout.Button("构建新的Cross状态表", GUILayout.Height(30))) {
            BuildCrossTable();
        }
        ...
        if (currentTable != null) {
            ...
            EditorGUILayout.LabelField($"状态数量: {currentTable.totalStates}");
    ...
    void BuildCrossTable()
    {
        if (tableCreator == null) {
            ...
            tableCreator = tempGO.AddComponent<CrossTableCreator>();
        }
    ...
    void SaveTableToAsset()
    {
        ...
        // 构建资产在项目中的相对路径
        string assetRelativePath = $"Assets/{folderName}/CrossStateTable.asset";
